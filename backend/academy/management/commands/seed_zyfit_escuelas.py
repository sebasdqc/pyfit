"""Seed de las 3 Escuelas de Zyfit Academy con sus cursos y módulos.
...
Uso:
    ...
    python manage.py seed_zyfit_escuelas --publicar  # publica los 9 cursos
"""
...
ESCUELAS = [
    ...
    },
    {
        ...
        'descripcion': (
            'Herramientas cuantitativas para medir, interpretar y optimizar el '
            'rendimiento del atleta con datos reales.'
        ),
        'orden': 2,
        'cursos': [
            {
    ...
    },
    {
        ...
        'descripcion': (
            'Ciencia aplicada al bienestar del atleta: monitoreo de HRV, prevención '
            'de lesiones y carga psicológica.'
        ),
        'orden': 3,
        'cursos': [
            {
...
class Command(BaseCommand):
    help = 'Crea (o sincroniza) las 3 escuelas de Zyfit Academy con sus cursos y módulos.'

    def add_arguments(self, parser):
        parser.add_argument(
            '--publicar',
            action='store_true',
            help='Publica los 9 cursos (publicado=True). Por defecto quedan como borradores.',
        )
    ...
    def handle(self, *args, **options):
        ...
        for escuela_data in ESCUELAS:
            ...
            for idx, curso_data in enumerate(cursos_data, start=1):
        ...
        self.stdout.write(self.style.SUCCESS(
            '\n✓ Seed completado: 3 escuelas, 9 cursos, 45 módulos (estructura).'
        ))
        if not publicar:
            self.stdout.write(
                '  Los cursos están en borrador. Usa --publicar para publicarlos.'
            )
