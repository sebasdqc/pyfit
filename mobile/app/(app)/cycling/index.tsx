...
import { generateRideSession, type GeneratedRide, type RideFase } from '../../../lib/cyclingApi'

// Mismo código de fases que running (calentamiento/principal/enfriamiento) —
// es una convención de producto, no algo propio de correr.
...
// Mismo vocabulario de ajuste que running (endurance.readiness.adapt_today
// es la MISMA tabla para los dos deportes) — mismas etiquetas.
...
export default function CyclingSessionScreen() {
  const { colors } = useTheme()
  const insets = useSafeAreaInsets()

  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [data, setData] = useState<GeneratedRide | null>(null)

  const generar = useCallback(async () => {
    setLoading(true)
    setError(null)
    try {
      setData(await generateRideSession())
    } catch (e: any) {
  ...
  // Sin DISTANCIA — ciclismo se prescribe en tiempo (ver ai_cycling.
  // training_science_cycling: no hay bifurcación distancia/tiempo).
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <ScrollView
        ...
        {/* Header */}
        <Text style={[s.eyebrow, { color: colors.accent }]}>TU SALIDA DE HOY</Text>
        ...
        {/* Fases */}
        {w.fases.map((fase, i) => (
          <FaseBlock key={`${fase.nombre}-${i}`} fase={fase} colors={colors} defaultOpen={i === 1} />
        ))}

        {/* CTA */}
        <TouchableOpacity
          onPress={() => router.replace(`/(app)/ride?planned=${data?.id ?? ''}`)}
          activeOpacity={0.88}
          style={{ marginTop: 20 }}>
          <LinearGradient
            ...
            style={s.cta}>
            <Text style={s.ctaText}>Empezar salida</Text>
          </LinearGradient>
        </TouchableOpacity>
...
function FaseBlock({ fase, colors, defaultOpen }: {
  fase: RideFase; colors: any; defaultOpen: boolean
}) {
      ...
      {open && fase.segmentos.map((seg, j) => (
        <View key={j} style={[s.segRow, { borderTopColor: colors.borderDefault }]}>
          ...
          <View style={s.chipsRow}>
            {/* potencia_objetivo suele ser null (sin potenciómetro) — el caso
                ESPERADO, no un estado degradado. fc_objetivo es el ancla. */}
            {seg.potencia_objetivo && <Chip text={seg.potencia_objetivo} color={colors.cyan} colors={colors} />}
