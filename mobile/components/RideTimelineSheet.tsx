/**
 * RideTimelineSheet.tsx — Hoja modal con la RUTINA COMPLETA de la salida de
 * ciclismo en curso: todos los `Paso[]` (ver lib/runSteps.ts) como timeline
 * vertical, no solo el bloque actual. Mismo componente/UX que
 * RunTimelineSheet.tsx (running) — misma lógica de offsets/estado/íconos
 * desde lib/runTimeline.ts, que ya es sport-agnostic (opera sobre `Paso`,
 * compartido por los dos motores). Lo único que cambia es el chip principal:
 * POTENCIA (W) en vez de ritmo — ciclismo se prescribe siempre en tiempo
 * (nunca metaDistanciaM), así que tampoco hay distinción indoor/outdoor ni
 * distancia total que mostrar en el subtítulo.
 *
 * ride/index.tsx no tiene mapa (decisión de producto: sin GPS en v1), así
 * que este sheet no compite por espacio con nada — pero se mantiene como
 * modal, igual que en running, para conservar la misma interacción.
 */
...
import {
  estadoPaso, offsetsAcumulados, metaTexto, TIPO_ICON, TIPO_COLOR_KEY,
} from '../lib/runTimeline'
import { formatDuration } from '../lib/runMetrics'

interface Chip { text: string; colorKey: keyof Colors }

function chipsDelPaso(paso: Paso): Chip[] {
  const chips: Chip[] = []
  if (paso.objetivo.powerRange) {
    chips.push({ text: `${paso.objetivo.powerRange[0]}–${paso.objetivo.powerRange[1]} W`, colorKey: 'cyan' })
  }
...
export function RideTimelineSheet({
  visible, onClose, pasos, pasoIdx, tituloSesion, duracionTotalMin, colors,
}: Props) {
  ...
  // Autoscroll al paso en curso cuando se abre la hoja.
  useEffect(() => {
    if (!visible) return
